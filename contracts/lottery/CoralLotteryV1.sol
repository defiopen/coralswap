...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Coral Lottery Ticket", "CLT") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function reset() external onlyAdmin {
        ...
        coral.transfer(destroyAddress,getTotalRewards(issueIndex-1).mul(burnDivisor).div(100));
    ...
    //添加externalRandomNumber以防止节点验证器被利用
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        coral.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        coral.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            coral.safeTransfer(address(msg.sender), reward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        coral.safeTransfer(address(msg.sender), _amount);
